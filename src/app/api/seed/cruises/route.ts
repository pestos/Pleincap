...
export async function GET() {
  ...
  log.push('🎯 Starting Cruises seed script...\n')

  for (const cruise of cruisesData) {
    try {
      log.push(`Processing: ${cruise.title}`)
      ...
      if (boatResult.docs.length > 0) {
        ...
        log.push(`  ↳ Resolved boat: ${cruise.ship} (ID: ${boatId})`)
      } else {
        log.push(`  ⚠ Boat not found: ${cruise.ship} (relationship will be empty)`)
      }
      ...
      if (cruise.destinationMatch) {
        ...
        if (destResult.docs.length > 0) {
          ...
          log.push(`  ↳ Resolved destination: ${destResult.docs[0].name} (ID: ${destinationId})`)
        } else {
          log.push(
        ...
        log.push(`  ⚠ No destination match defined (relationship will be empty)`)
      }
      ...
      if (cruise.detailData) {
        log.push(`  ↳ Adding detailed itinerary (${danubeItineraryData.length} days)`)
        ...
        log.push(`  ↳ Resolving speakers (${danubeExperts.length} experts)`)
        const speakerIds = []
        for (const expertName of danubeExperts) {
          ...
          if (speakerResult.docs.length > 0) {
            ...
            log.push(`    ✓ Resolved speaker: ${expertName}`)
          } else {
            log.push(`    ⚠ Speaker not found: ${expertName}`)
          }
        }
  ...
  log.push('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
  log.push('📊 Cruises Migration Summary')
  log.push('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
  log.push(`Total cruises: ${cruisesData.length}`)
